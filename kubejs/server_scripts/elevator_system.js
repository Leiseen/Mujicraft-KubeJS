...
const playerOnElevator = {}; // 记录玩家是否在电梯上

// 辅助函数：检查玩家是否在冷却中
function isOnCooldown(playerUUID) {
    if (!playerCooldowns[playerUUID]) return false;
    const now = Date.now();
    const cooldownEnd = playerCooldowns[playerUUID];
    return now < cooldownEnd;
}

// 辅助函数：设置玩家冷却
function setCooldown(playerUUID) {
    playerCooldowns[playerUUID] = Date.now() + (ELEVATOR_CONFIG.cooldownSeconds * 1000);
}

// 辅助函数：获取剩余冷却时间
function getRemainingCooldown(playerUUID) {
    if (!playerCooldowns[playerUUID]) return 0;
    const remaining = Math.ceil((playerCooldowns[playerUUID] - Date.now()) / 1000);
...
function findElevatorFloor(player) {
    ...
    const py = Math.floor(player.y) - 1;
    ...
    // 检查 1F
    if (px === 125 && py === 48 && pz === 207) {
        if (ELEVATOR_CONFIG.debugMode) {
            console.log(`[电梯调试] ✅ 找到匹配楼层: 1F-底层`);
        }
        return {
            floorIndex: 0,
            floor: { level: 1, name: "1F-底层", x: 125, y: 48, z: 207 },
            nextFloor: { level: 2, name: "2F-二层", x: 125, y: 52, z: 207 },
            prevFloor: null,
            isBottom: true,
            isTop: false
        };
    ...
    // 检查 2F
    if (px === 125 && py === 52 && pz === 207) {
        if (ELEVATOR_CONFIG.debugMode) {
            console.log(`[电梯调试] ✅ 找到匹配楼层: 2F-二层`);
        }
        return {
            floorIndex: 1,
            floor: { level: 2, name: "2F-二层", x: 125, y: 52, z: 207 },
            nextFloor: { level: 3, name: "3F-三层", x: 125, y: 59, z: 207 },
            prevFloor: { level: 1, name: "1F-底层", x: 125, y: 48, z: 207 },
            isBottom: false,
            isTop: false
        };
    }
    ...
    // 检查 3F
    if (px === 125 && py === 59 && pz === 207) {
        if (ELEVATOR_CONFIG.debugMode) {
            console.log(`[电梯调试] ✅ 找到匹配楼层: 3F-三层`);
        }
        return {
            floorIndex: 2,
            floor: { level: 3, name: "3F-三层", x: 125, y: 59, z: 207 },
            nextFloor: { level: 4, name: "4F-顶层", x: 125, y: 66, z: 207 },
            prevFloor: { level: 2, name: "2F-二层", x: 125, y: 52, z: 207 },
            isBottom: false,
            isTop: false
        };
    }
    ...
    // 检查 4F
    if (px === 125 && py === 66 && pz === 207) {
        if (ELEVATOR_CONFIG.debugMode) {
            console.log(`[电梯调试] ✅ 找到匹配楼层: 4F-顶层`);
        }
        return {
            floorIndex: 3,
            floor: { level: 4, name: "4F-顶层", x: 125, y: 66, z: 207 },
            nextFloor: null,
            prevFloor: { level: 3, name: "3F-三层", x: 125, y: 59, z: 207 },
            isBottom: false,
            isTop: true
        };
    }
...
PlayerEvents.tick(event => {
    ...
    // 检查玩家脚下是否是电梯方块
    const blockBelow = player.block.down;
    const isOnTriggerBlock = blockBelow.id === ELEVATOR_CONFIG.triggerBlock;
    ...
    if (isOnElevator) {
        // 在电梯上，显示提示
        if (!playerOnElevator[playerUUID]) {
            playerOnElevator[playerUUID] = true;
        }
        ...
        if (playerOnElevator[playerUUID]) {
            ...
            playerOnElevator[playerUUID] = false;
        }
    }
    ...
    const isJumping = player.motionY > 0.08; // 进一步降低阈值，更容易触发
...
PlayerEvents.loggedIn(event => {
    ...
    event.server.scheduleInTicks(60, () => {
        if (Object.keys(global.elevatorFloors).length > 0) {
